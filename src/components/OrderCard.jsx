...
const OrderCard = ({ order, index }) => {
  ...
  return (
    <div className="mb-6 overflow-hidden rounded-lg border border-primary-100 shadow-sm">
      {/* 卡片標題區域，顯示訂單編號 */}
      <div className="flex items-center justify-between bg-primary-200 p-4 text-lg font-normal text-primary-700">
        <p>第 {index + 1} 筆</p>
        <button
      ...
      {/* 詳細內容區域，只有展開時才顯示 */}
      {isOpen && (
        <div className="flex flex-col border-t border-grey-100 md:flex-row">
          {/* 導遊資料區 */}
          <div className="flex flex-col items-center justify-center  bg-primary-100 p-6 md:w-1/3 md:border-b-0 md:border-r">
            <img
              src={order.tourguideInfo.imgUrl || "Loading..."}
              alt="導遊"
              className="inline-block h-20 w-20 rounded-full"
            />
            <p className="mt-4 text-xl text-gray-700 font-normal">
